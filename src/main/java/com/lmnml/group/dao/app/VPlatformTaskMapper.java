...
public interface VPlatformTaskMapper extends MyMapper<VPlatformTask> {
    ...
    @Select("SELECT vps.id,vps.name,vps.price*((100-vps.ratio)/100) price,vps.num,vps.last_num lastNum,DATE_FORMAT(vps.end_time,'%Y年%m月%d日 %H:%i') endTime,vps.check_time checkTime\n" +
    ...
    @Select("SELECT vpt.name,vpt.category,DATE_FORMAT(vpt.create_time,'%Y/%m/%d') createTime,DATE_FORMAT(vpt.end_time,'%Y年%m月%d日 %H:%i') endTime,vpt.status,vpt.device_type deviceType,vpt.submit_type submitType,vpt.last_num lastNum,vpt.price*((100-vpt.ratio)/100) price\n" +
            "FROM v_platform_task vpt\n" +
            "WHERE id=#{taskId}")
    ...
    @Select("SELECT price*num FROM v_platform_task WHERE id=#{taskId}")
