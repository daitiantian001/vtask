...
//            String accessKeySecret = "Z4ngAnXvwiBnA9Xm6RrkukNsWjOlgi";
            String endpoint = "http://oss-cn-beijing.aliyuncs.com";
            String accessKeyId = "LTAIS6J7F6iTmlb2";
            String accessKeySecret = "01rnURnm2EK0zIqmS7zatDsyBHTLlp";
            OSSClient ossClient = new OSSClient(endpoint, accessKeyId, accessKeySecret);
            String id = StrKit.ID();
            String url = String.format("%s%s.%s", "http://feitengtask.oss-cn-beijing.aliyuncs.com/", id, ext);
            InputStream inputStream = null;
            inputStream = file.getInputStream();
//            ossClient.putObject("yuejinimg", id + "." + ext, inputStream);
            ossClient.putObject("feitengtask", id + "." + ext, inputStream);
            ossClient.shutdown();
